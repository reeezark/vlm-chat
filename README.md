...
- 点击“校验配置”会检查 Provider、API 地址、API Key 和模型名是否满足必填与格式要求。
- 点击“测试连接”会调用 `/api/model-config/test`，验证当前配置是否能实际访问模型服务。
- “当前配置状态”会展示实际生效的 Provider、模型、Base URL、Key 来源和测试耗时，不展示 API Key 原文。
